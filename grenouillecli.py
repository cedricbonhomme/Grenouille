...
def send_measure(url, data, email, password):
    ...
    r = requests.post(
        url, data=json.dumps(data), headers=headers, auth=(email, password)
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Client to send weather data to the Grenouille platform."
    )
    parser.add_argument(
        "--url",
        dest="url",
    ...
    parser.add_argument("--email", dest="email", required=True, help="Your email")
    parser.add_argument(
        "--password", dest="password", required=True, help="Your password"
    )
    parser.add_argument("--api-key", dest="key", required=True, help="Your API key")
    parser.add_argument(
        "--station", dest="station", type=int, required=True, help="Id of the station"
    )
