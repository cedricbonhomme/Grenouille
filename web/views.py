...
def edit_station(station_id=None):
    ...
    if request.method == "GET":
        ...
        return render_template(
            "edit_station.html", user=user, form=form, message=message
        )
...
def download_station(station_id):
    ...
    r.headers["Content-Disposition"] = (
        "attachment; filename=measures_station_" + str(station.id) + ".json"
    )
    return r
...
def disable_user(user_id=None):
    ...
    if user is not None:
        if user.apikey != "":
            user.apikey = ""
            flash(
                'API key of the user "' + user.firstname + '" successfully disabled.',
                "success",
            )
        else:
            import random, base64, hashlib

            user.apikey = base64.b64encode(
                hashlib.sha512(str(random.getrandbits(256))).digest(),
                random.choice(["rA", "aZ", "gQ", "hH", "hG", "aR", "DD"]),
            ).rstrip("==")
            flash(
                'API key of the user "' + user.firstname + '" successfully enabled.',
                "success",
            )
        db.session.commit()
    else:
